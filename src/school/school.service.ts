...
export class SchoolService {
    ...
    async createSchool( schoolDto: SchoolDto): Promise<School> {
        try{
            ...
            throw new Error('No se pudo cargar la escuela')
        } catch(error) {
            throw new HttpException( { status : HttpStatus.BAD_REQUEST,
            error: 'Error en la creación de escuela' + error},
    ...
    async updateSchool(id: number, schoolDto: SchoolDto): Promise<School> {
        try {
            ...
            if (!updateSchool) throw new Error('No se encuentra una escuela con id:' + id),
